...
- `GET /v1/models` 返回 `gpt-image-2`、`codex-gpt-image-2`、`auto`、`gpt-5`、`gpt-5-1`、`gpt-5-2`、`gpt-5-3`、`gpt-5-3-mini`、
  `gpt-5-mini`
- `/v1/models` 会合并账号侧可用模型；上游模型列表异常时会保底返回图片模型，避免前端选择器空白
- 支持通过 `n` 返回多张生成结果
- 支持生成可编辑 PPT 文件
- 支持生成可编辑 PSD 文件
...
- 前端会从 `/v1/models` 拉取真实模型列表，并在接口异常时回退到可用默认模型
- 编辑模式支持参考图上传
- 前端支持多图生成交互
- 本地保存图片会话历史，支持回看、删除、重命名和清空
- 支持移动端响应式画图界面，历史入口、参数设置、图片结果和快捷操作针对手机屏幕做了重排
- 移动端图片参数设置使用底部抽屉，避免浮层在窄屏溢出
- 发送新提示词后结果区会自动滚动到最新任务
- 支持服务端缓存图片URL
- 图片生成进度追踪，超时后可继续等待
- 服务重启后会尝试从已落盘图片恢复未完成任务结果，减少“前端失败但图片管理成功”的状态不一致
...
- 账号、日志、图片管理、设置、注册和实验室页面做了移动端布局适配，减少窄屏溢出和单字换行

### Web 管理端体验

- 使用全局登录态管理，页面切换不再重复阻塞式校验 `/auth/login`
- 内部导航使用客户端路由，移动端抽屉菜单切换页面时减少整页刷新
- 关键首屏接口设置超时，后端慢响应时会退出 loading 并给出错误反馈
- 实验室对话面板支持模型下拉选择，并保留 tab 切换前后的状态
...
| 字段   | 说明                                                                                                         |
|:-----|:-----------------------------------------------------------------------------------------------------------|
| 返回模型 | 以接口实时返回为准，默认包含图片模型，并会合并账号侧发现的可用模型 |
